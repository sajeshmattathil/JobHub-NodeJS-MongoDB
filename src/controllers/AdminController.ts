...
export class AdminController {
  ...
  async loginSubmit(req: Request, res: Response): Promise<void> {
    try {
  ...
  async blockUnblockUser(req: Request, res: Response): Promise<void> {
    try {
      const response = await this.interactor.blockUblockUser(
        req.body.email,
        req.body.isBlocked
      );
      if (response.status === 201) {
  ...
  async hiringManagers(req: Request, res: Response): Promise<void> {
    try {
      ...
      const response = await this.interactor.getHiringManagers(
        Number(pageNumber),
        Number(HRsPerPage)
      );
  ...
  async hiringmanagersApproved(req: Request, res: Response): Promise<void> {
    try {
      ...
      const response = await this.interactor.getHiringManagersApproved(
        Number(pageNumber),
        Number(HRsPerPage)
      );
      if (response !== undefined) {
  ...
  async blockUnblockHR(req: Request, res: Response): Promise<void> {
    try {
      const response = await this.interactor.blockUnblockHR(
        req.body.email,
        req.body.isBlocked
      );
      if (response.status === 201) {
  ...
  async updatePlan(req: Request, res: Response): Promise<void> {
    try {
      const { planId } = req.params;
      const response = await this.interactor.updatePlan(
        String(planId),
        req.body
      );
  ...
  async getDashboardData(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (dashboardData.status === 202)
        res.status(202).json({ dashboardData: dashboardData?.data });
